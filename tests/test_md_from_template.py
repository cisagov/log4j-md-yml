...
# cisagov Libraries
import md_from_template
import md_from_template.md_from_template
...
PROJECT_VERSION = md_from_template.__version__
...
def test_stdout_version(capsys):
    ...
    with pytest.raises(SystemExit):
        with patch.object(sys, "argv", ["bogus", "--version"]):
            md_from_template.md_from_template.main()
...
def test_running_as_module(capsys):
    ...
    with pytest.raises(SystemExit):
        with patch.object(sys, "argv", ["bogus", "--version"]):
            ...
            # cisagov Libraries
            import md_from_template.__main__  # noqa: F401
    captured = capsys.readouterr()
    assert (
        captured.out == f"{PROJECT_VERSION}\n"
    ), "standard output by '--version' should agree with module.__version__"
